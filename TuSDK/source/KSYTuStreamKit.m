...
#import "KSYTuStreamKit.h"

#define FLOAT_EQ( f0, f1 ) ( (f0 - f1 < 0.0001)&& (f0 - f1 > -0.0001) )

@interface KSYTuStreamKit (){
...
// 组装视频通道
- (void) setupVideoPath {
    __weak KSYTuStreamKit * kit = self;
    // 前处理 和 图像 mixer
...
int tu_UIOrienToIdx (UIInterfaceOrientation orien) {
    switch (orien) {
...
- (void) rotatePreviewTo: (UIInterfaceOrientation) orie {
    dispatch_async(dispatch_get_main_queue(), ^(){
        ...
        if (_videoOrientation == orie || view == nil || _vCapDev.isRunning == NO ) {
            ...
            int capOri = tu_UIOrienToIdx(_videoOrientation);
            int appOri = tu_UIOrienToIdx(orie);
